...
Block_Encryptor::~Block_Encryptor()//Неявно вызовет деструктор род класса, поэьтому должен быть объявлен как virtual
{
    key = "";
    len_key = 0;
}
...
void Block_Encryptor::encript() //Функция кодировки
{
    ...
    for (size_t j = ((len_info / len_key) * len_key), z = 0; j < len_info; ++j, ++z) //Проход по остатку от блоков справа
    {
